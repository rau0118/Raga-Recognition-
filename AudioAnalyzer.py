...
original = AudioAnalyzer("audio/bupali1.wav", input_sr=44100, fft_size=44100)

remaster_09 = AudioAnalyzer("audio/bupali2.wav", input_sr=44100, fft_size=44100)


original.plot_spectrum(min_freq=20, max_freq=1000, title="Original")

remaster_09.plot_spectrum(min_freq=20, max_freq=1000, title="testcase1")


orig_to_09 = SpectrumCompare(original, remaster_09)
orig_to_09.plot_spectrum_group(
    frange=(20,1000), 
    ratio=True, 
    threshold=True,
    title="Spectrograms - original vs testcase1 (with threshold)", 
    legend=("Original", "testcase1", "Amplitude Diff.", "Threshold")
    )
orig_to_09.plot_spectrum_heatmap(
    frange=(20,500), 
    plot_spec1=False, 
    title="Original vs testcase1"
    )
